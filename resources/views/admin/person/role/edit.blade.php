...
@section('main')
    ...
    <div class="d-flex flex-column">
        <x-container class="w-50">
            <form action="{{ route('admin.person.role.update', ['role_id' => $role->id]) }}" method="post">
                @csrf
                @method("PUT")
                <div class="input-group mb-3">
                    <span class="input-group-text" id="inputGroup-sizing-default">Name</span>
                    <input type="text" class="form-control @error('name') is-invalid @enderror"
                           aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default" name="name" value="{{ old('name', $role->name) }}">
                    @error('name')
