...
@section('main')
    ...
    <div class="d-flex flex-column">
        <x-container class="w-50">
            ...
            <form action="{{ route('create-person-role.action') }}" method="post">
                @csrf
            ...
            <table class="table table-hover table-bordered">
                ...
                @foreach($roles as $index => $role)
                    <tr>
                        ...
                        <td>
                            <div class="d-flex">
                                <a href="{{ route('admin-edit-person-role.page', ['role_id' => $role->id]) }}"
                                   class="me-1">
                                    <button class="btn btn-sm btn-outline-success">
                                        <i class="bi bi-pencil"></i>
                                    </button>
                                </a>
                                <form action="{{ route('delete-person-role.action', ['role_id' => $role->id]) }}" method="post">
                                    @csrf
                                    @method("DELETE")
