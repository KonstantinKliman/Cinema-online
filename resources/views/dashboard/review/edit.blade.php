...
@section('main')
    ...
    <div class="d-flex flex-column">
        <x-container class="w-50">
            <form action="{{ route('dashboard.review.update', ['review_id' => $review->id]) }}" method="post">
                @csrf
                @method("PUT")
                <div class="input-group mb-3">
                    ...
                    <select name="type" class="form-select">
                        ...
                        @foreach($review->getReviewType() as $reviewValue => $reviewType)
                            <option
                                value="{{ $reviewValue }}" {{ $review->type == $reviewValue ? 'selected' : '' }}>{{ ucfirst($reviewType) }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="input-group mb-3">
                ...
                <div class="form-check form-switch">
                    <input class="form-check-input" name="is_published" type="checkbox"
                           id="flexSwitchCheckDefault" {{ $review->is_published ? 'checked' : '' }}>
