...
@section('main')
    <div class="d-flex flex-row align-items-center mt-3">
        ...
        <a href="{{ route('dashboard.person.edit', ['person' => $person->slug]) }}" class="ms-1">
        ...
        <div class="col">
            <table class="table table-hover table-bordered">
                ...
                @foreach($movies as $index => $movie)
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td><a href="{{ route('dashboard.movie.show', ['movie_id' => $movie->id]) }}"
                        ...
                        <td>
                            <div class="row d-flex flex-column">
                                @foreach($movie->personRoles()->where('person_id', $person->id)->get() as $roles)
                                    <div class="col mb-1">
                                        <form
                                            action="{{ route('dashboard.person.detach', ['person' => $person->slug, 'movie_id' => $movie->id, 'role_id' => $roles->role->id,]) }}"
                                            method="post">
                                            @csrf
                                            {{ $roles->role->name }}
                        ...
                        <td><a href="{{ route('dashboard.user.show', ['user_id' => $movie->user->id]) }}"
