...
@section('main')
    ...
    <x-container>
        <form action="{{ route('dashboard.movie.update', ['movie_id' => $movie->id]) }}" method="post"
            ...
            <div class="input-group mb-3">
                ...
                <input type="text" class="form-control @error('production_studio') is-invalid @enderror"
                       ...
                       name="production_studio" value="{{ $movie->production_studio }}">
            ...
            <div class="input-group mb-3">
                ...
                <input type="number" min="1895" max="{{ now()->year }}"
                       class="form-control @error('release_year') is-invalid @enderror"
                       aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default"
                       name="release_year" value="{{ $movie->release_year }}">
                @error('release_year')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
                ...
                <select class="form-control" multiple="" aria-label="Multiple select example" size="5"
                    ...
                    @foreach($genres as $genre)
                        <option value="{{ $genre->id }}"
                                @foreach($movie->genres as $movieGenre)@if($movieGenre->id == $genre->id) selected @endif @endforeach>{{ $genre->name }}</option>
                    @endforeach
                </select>
                @error('genres[]')
                <div class="invalid-feedback">
